<template>
  <div
    class="flex flex-col justify-between p-4 overflow-hidden bg-white rounded-lg md:p-6"
  >
    <div class="flex items-start justify-between">
      <div>
        <h5 class="md:text-xl text-base font-bold text-Black"> {{ car.name }} </h5>
          <p class="font-bold md:text-sm text-DarkGray text-xs"> {{ car.type }} </p>
      </div>
      <div>
    ...
    <div class="md:block flex justify-between">
      <div class="md:pt-12 md:pb-10 pt-8 pb-6 max-w-60 mx-auto md:w-auto w-2/3">
        <div>
          <img
            :src="car.img"
            alt="Car"
            class="block md:h-26 md:max-w-full h-20	max-w-40 mx-auto object-contain"
          />
        </div>
        <div class="md:-mt-12 -mt-8 w-full">
          <svg
            width="304"
            class="w-full md:h-16 h-10"
            viewBox="0 0 304 68"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
      ...
      <div class="md:w-auto w-1/3 md:pl-0 pl-3">
        <ul
          class="flex md:flex-row flex-col md:items-center md:gap-4 gap-2 pb-0 md:pb-5"
        >
          <li>
            <p class="flex items-center gap-1">
              <svg
              class="md:w-5	w-3.5 md:h-5 h-3.5"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
              ...
              <span class="text-sm font-medium text-DarkGray">{{ car.gasolineLiter}}L</span>
            </p>
          </li>
          <li>
            <p class="flex items-center gap-1">
              ...
              <span class="text-sm font-medium text-DarkGray"
                >{{ car.kindOfTransition }}</span
              >
            </p>
          </li>
          <li>
    ...
    <div>
      <div class="grid items-center grid-cols-2 md:gap-4 gap-2">
          <div>
            <p class="md:text-xl text-base font-bold text-Black"
              >${{ car.pricePerDay }}<span class="md:text-sm text-xs font-bold text-DarkGray"
                >/ day</span
              ></p
            >
          </div>
          <div class="text-right">
            <nuxt-link :to="`/cars/${car.id}`" class="md:px-5 md:py-3 py-2 btn btn-blue px-4">Rent Now</nuxt-link>
          </div>
        </div>
    </div>
  </div>
